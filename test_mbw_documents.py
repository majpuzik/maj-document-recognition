...
class MBWDocumentTester:
    ...
    def __init__(self, mbw_dir: str):
        ...
        self.voter = AIVoter()
        self.learning_db = LearningDatabase('mbw_learning_patterns.jsonl')
    ...
    def find_mbw_documents(self) -> List[Path]:
        """Find all MBW PDF documents"""
        pdf_files = list(self.mbw_dir.glob("*MBW*.pdf"))
        pdf_files.extend(self.mbw_dir.glob("*mbw*.pdf"))

        # Remove duplicates
        pdf_files = list(set(pdf_files))

        logger.info(f"📄 Found {len(pdf_files)} MBW PDF documents")
...
def main():
    ...
    mbw_dir = Path.home() / "Library/CloudStorage/OneDrive-Osobní/1allpdf"
    ...
    # Save results
    results_file = 'mbw_test_results.json'
    with open(results_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print(f"\n💾 Results saved: {results_file}")
    print(f"💾 Learning patterns: mbw_learning_patterns.jsonl")
    print("\n✅ MBW test complete!")

...
"""
Quick test: Process first 100 documents WITH metadata
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/process_100_test.log"),
...
def main():
    """Process first 100 files from temp_attachments directory WITH metadata"""
    start_time = time.time()

    logger.info("="*80)
    logger.info("📄 TEST: PROCESSING 100 DOCUMENTS WITH EMAIL METADATA")
    logger.info("="*80)
    logger.info("Optimizations:")
    logger.info("  1. OCR CASCADE (ces → eng → deu)")
    logger.info("  2. FAST PRE-CLASSIFIER")
    logger.info("  3. DOCUMENT CACHE")
    logger.info("  4. PARALLEL WORKERS (4)")
    ...
    if not metadata_file.exists():
        ...
        logger.error("   Run extract_thunderbird_simple.py first!")
        sys.exit(1)
    ...
    # Take only first 100
    attachments_data = attachments_data[:100]
    logger.info(f"✓ Processing first 100 documents")
    ...
    with ProcessPoolExecutor(max_workers=initial_workers) as executor:
        ...
        for future in as_completed(futures_list):
            try:
                ...
                if completed % 10 == 0:
                    logger.info(f"📊 Progress: {completed}/{len(attachments)}")

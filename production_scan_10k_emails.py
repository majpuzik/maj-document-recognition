...
"""
Production Email Scanner - 10,000 Emails
==========================================
Scans 10,000 emails from Thunderbird INBOX
...
class ProductionEmailScanner:
    """Production email scanner with AI Consensus V2"""

    def __init__(self, mbox_path: str, output_dir: str, max_emails: int = 10000):
        ...
        logger.info("🚀 Initializing Production Email Scanner V2")
    ...
    def scan_mbox(self) -> List[Tuple[int, email.message.EmailMessage]]:
        """Scan mbox for first N emails with PDF attachments"""

        logger.info(f"📧 Scanning mbox: {self.mbox_path.name}")
        ...
        for idx, msg in enumerate(mbox):
            if idx >= self.max_emails:
                logger.info(f"   Reached max emails limit: {self.max_emails}")
                break

            if idx % 1000 == 0 and idx > 0:
                logger.info(f"   Processed {idx} emails, found {len(emails_with_pdfs)} with PDFs...")
    ...
    def run(self):
        ...
        logger.info("\n" + "=" * 80)
        logger.info("🚀 PRODUCTION EMAIL SCANNER V2 - STARTING")
        logger.info("=" * 80)
        ...
        logger.info("📧 PHASE 1: Scanning emails for PDF attachments...")
    ...
    def save_results(self):
        ...
        output_file = self.output_dir / 'production_scan_results.json'
...
def main():
    ...
    # Thunderbird INBOX path - 125,118 emails!
    mbox_path = Path.home() / "Library/Thunderbird/Profiles" / \
                "1oli4gwg.default-esr/ImapMail/127.0.0.1/INBOX"

    # Output directory
    output_dir = Path(__file__).parent / "production_scan_output"

    # Create scanner
    scanner = ProductionEmailScanner(
